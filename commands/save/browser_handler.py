...
class BrowserHandler:
    def __init__(self, debug: bool = False) -> None:
        self.debugger = CommandDebugger("BrowserHandler", debug)
    ...
    def is_zen_browser_window(self, window_data):
        ...
        self.debugger.debug(f"Checking window class '{class_name}' -> is_zen: {is_zen}")
        return is_zen
    ...
    def get_browser_session_info(self, window_data):
        ...
        try:
            ...
            self.debugger.debug(f"Browser session info: {session_data}")
            return session_data

        except subprocess.TimeoutExpired as e:
            self.debugger.debug(f"Timeout getting browser session info: {e}")
            return None
        except subprocess.CalledProcessError as e:
            self.debugger.debug(f"Command failed getting browser session info: {e}")
            return None
        except Exception as e:
            self.debugger.debug(f"Unexpected error getting browser session info: {e}")
            return None

    def create_basic_session_data(self, window_data):
        """Create basic session data for browser without extension support"""
        self.debugger.debug("Creating basic session data (no extension)")
    ...
    def wait_for_keyboard_shortcut_file(self, timeout=None):
        ...
        self.debugger.debug(f"Monitoring {downloads_dir} for keyboard shortcut tab files")
        ...
        try:
            ...
            self.debugger.debug(f"Initial tab files: {len(initial_files)}")
        except (OSError, PermissionError) as e:
            self.debugger.debug(f"File system error getting initial file list: {e}")
        except Exception as e:
            self.debugger.debug(f"Unexpected error getting initial file list: {e}")
            ...
            try:
                ...
                if new_files:
                    ...
                    try:
                        ...
                        self.debugger.debug(f"File system error reading file: {e}")
                    except json.JSONDecodeError as e:
                        self.debugger.debug(f"JSON decode error reading file: {e}")
                    except Exception as e:
                        self.debugger.debug(f"Unexpected error reading file: {e}")

            except (OSError, PermissionError) as e:
                self.debugger.debug(f"File system error checking for new files: {e}")
            except Exception as e:
                self.debugger.debug(f"Unexpected error checking for new files: {e}")
    ...
    def load_keyboard_shortcut_tab_data(self, tab_file_path):
        ...
        try:
            ...
            self.debugger.debug(f"Loaded {len(tabs)} tabs from keyboard shortcut file")
            self.debugger.debug(
                f"Tab capture method: {tab_data.get('captureMethod', 'unknown')}"
            )
            self.debugger.debug(f"Window ID: {tab_data.get('windowId', 'unknown')}")
            ...
            self.debugger.debug(f"File system error loading keyboard shortcut tab file: {e}")
            return None
        except json.JSONDecodeError as e:
            self.debugger.debug(f"JSON decode error loading keyboard shortcut tab file: {e}")
            return None
        except Exception as e:
            self.debugger.debug(f"Unexpected error loading keyboard shortcut tab file: {e}")
            return None
    ...
    def capture_tabs_via_keyboard_shortcut(self, window_data):
        ...
        try:
            ...
            if not window_address:
                self.debugger.debug("No window address available for sendshortcut")
                return None
            ...
            if tab_file:
                ...
                if tab_data:
                    ...
                    try:
                        os.remove(tab_file)
                        self.debugger.debug(f"Cleaned up tab file: {tab_file}")
                    except:
                        pass
                    return tab_data
            ...
            self.debugger.debug(f"File system error in keyboard shortcut tab capture: {e}")
            return None
        except subprocess.TimeoutExpired as e:
            self.debugger.debug(f"Timeout in keyboard shortcut tab capture: {e}")
            return None
        except Exception as e:
            self.debugger.debug(f"Unexpected error in keyboard shortcut tab capture: {e}")
            return None
    ...
    def get_restore_command(self, browser_data, browser_type="zen"):
        ...
        self.debugger.debug(f"Generating restore command for browser type: {browser_type}")
        ...
        if browser_type == "zen":
            ...
            self.debugger.debug(f"Unknown browser type: {browser_type}, using zen as default")
            base_command = zen_command
        ...
        if tabs and len(tabs) > 0:
            ...
            if urls:
                self.debugger.debug(f"Adding {len(urls)} URLs to browser command")

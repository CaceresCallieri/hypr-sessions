...
class LaunchCommandGenerator:
    ...
    def guess_launch_command(self, window_data: WindowInfo) -> str:
        ...
        if browser_session:
            ...
            return self.browser_handler.get_restore_command(
                browser_session, browser_type
            )

        # Check for Neovide session data
        neovide_session = window_data.get("neovide_session")
        if neovide_session:
            return self.neovide_handler.get_restore_command(
                window_data, neovide_session.get("session_file")
            )
